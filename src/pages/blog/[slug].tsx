...
import { Box, Heading, Text, Container, Image, Link, Code } from '@chakra-ui/react'
...
export default function BlogPost({ post, mdxSource }: { post: Post, mdxSource: MDXRemoteSerializeResult }) {
   const { t } = useTranslation();

  return (
    <Container maxW="4xl" py={10}>
      <Head>
        <title>{`${t(post.title)} | Googlementor`}</title>
        <link rel="canonical" href={`https://googlementor.com/blog/${post.slug}`} />
      </Head>
      <Heading as="h1" size="xl" mb={4}>{t(post.title)}</Heading>
      <Text fontSize="sm" color="gray.500" mb={2}>
        {new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
      </Text>
      <Text fontSize="md" color="gray.700" mb={4}>{t(post.summary)}</Text>
