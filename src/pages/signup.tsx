...
const SignUp: React.FC = () => {
  ...
  const metaDescription = getStaticMetaDescription('signup', currentLang);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-3xl font-bold mb-6 text-gray-800 text-center">{t('signupPage.title', 'Join Our Team')}</h1>
        <p className="text-gray-700 text-center mb-6">
          {t('signupPage.intro', 'We are always looking for passionate individuals to join our team at Googlementor. If you\'re interested, please reach out to us at:')}
        </p>
        ...
        <p className="text-gray-700 text-center">
          {t('signupPage.outro', 'Send us an email with your details, and we\'ll get back to you as soon as possible!')}
        </p>
        <Link href="/" className="inline-block mt-6 px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow hover:bg-blue-700 transition-colors duration-300 text-center">
          {t('signupPage.returnHome', 'Return to Home')}
        </Link>
      </div>
    </div>
  );
